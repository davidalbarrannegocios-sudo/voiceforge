...
import { Search, Play, Square, ChevronDown, Loader2 } from "lucide-react";
...
function getAuthorName(voice: FishVoice): string {
  if (!voice.author) return "";
  if (typeof voice.author === "string") return voice.author;
  return voice.author.nickname ?? voice.author.handle ?? "";
}
...
export default function VoicesClient() {
  ...
  /* Audio preview — fetches URL via our server proxy to hide API key */
  ...
  // sticky offset: 64px header when no sidebar, 0 when sidebar is shown
  const stickyTop = showSidebar ? "top-0" : "top-16";
  const heroPadding = showSidebar ? "pt-16" : "pt-36";
  ...
  return (
    <>
      ...
      <div className="flex" style={{ minHeight: "100vh", background: "#000000", color: "#ffffff" }}>
        ...
        {/* ── Dashboard sidebar (authenticated users only) ──────── */}
        {showSidebar && (
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* ── Public navbar (unauthenticated only) ─────────────── */}
          {!showSidebar && (
            <header
          ...
          {/* ── Hero ─────────────────────────────────────────────── */}
          <section
            className={`relative overflow-hidden ${heroPadding} pb-16 px-4`}
          ...
          {/* ── Tag pills ────────────────────────────────────────── */}
          <div
            className={`sticky ${stickyTop} z-40 border-b overflow-x-auto`}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {voices.map((voice) => (
                  <VoiceCard
                    ...
                    isLoadingPreview={previewLoading === voice._id}
          ...
          {/* ── Footer ───────────────────────────────────────────── */}
          <footer className="border-t mt-16" style={{ borderColor: "#1a1a1a", background: "#000000" }}>
...
function VoiceCard({
  ...
  const gradient = generateVoiceGradient(voice._id);

  return (
    <div
      className="flex items-start gap-3 p-4 rounded-xl transition-all cursor-default"
    ...
    >
      {/* Avatar — gradient based on voice ID */}
      <div
      ...
      <div className="flex flex-col gap-2 flex-shrink-0">
        <button
          ...
          style={{ background: isPlaying ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.1)" }}
